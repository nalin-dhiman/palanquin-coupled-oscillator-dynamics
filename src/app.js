...
const walkingPresets=[['G90','Roll + pitch','A quarter-stride offset between front and rear: both roll and pitch respond.'],['G0','Same rhythm','Front and rear use the same stride phase.'],['G180','Hidden loads','Opposite torso sway can redistribute shoulder loads with almost no net rocking.'],['Gvar','Drifting rhythm','Slightly different cadences and shoulder amplitudes let the relative phase drift.'],['heave','Up / down','Matched vertical inputs, with torso tilt switched off.'],['roll','Sideways tilt','Matched torso sway, with vertical bounce switched off.'],['pitch','Front / back tilt','Front and rear vertical inputs oppose; torso tilt is switched off.'],['Gstress','Contact stress','Large, synthetic inputs test unloading. This is not a typical measured gait.'],['G90slow','Slower cadence','The quarter-stride example at 1.83 steps/s.'],['G90fast','Faster cadence','The quarter-stride example at 2.33 steps/s.']];
const legacyPresets=[['in-phase','In phase','Matched supports; a small initial roll decays.'],['offset','60° offset','Different phases on the two pole sides.'],['feedback','Load yielding','The original passive load-yielding rule.'],['irregular','Irregular inputs','A prescribed frequency mixture.'],['quiet','Quiet supports','No prescribed support movement.']];
...
const walkingGroups=[
 ['Walking rhythm',false,[['mode','Pitch comparison','','select',[['full','Four contacts · free pitch'],['pitch_locked','Four contacts · locked pitch'],['collapsed','Two averaged resultants']]],...['Front','Rear'].flatMap((s,j)=>[f(`gait.step_hz.${j}`,`${s} step cadence`,'steps/s',.1,5,.01,1,true),f(`gait.stride_phase_rad.${j}`,`${s} stride phase`,'degrees',-360,360,1,DEG,true)])]],
 ['Shoulder motion',true,[...['Front','Rear'].flatMap((s,j)=>[f(`gait.vertical_m.${j}`,`${s} vertical bounce`,'mm',0,100,.5,1000,true),f(`gait.tilt_rad.${j}`,`${s} torso tilt`,'degrees',0,15,.1,DEG,true)]),f('gait.ramp_s','Smooth start','s',.1,30,.1)]],
...
function modeLabel(c){return !isWalking(c)?'Original two supports':({full:'Four contacts · free pitch',pitch_locked:'Four contacts · locked pitch',collapsed:'Two averaged resultants'})[c.mode];}
function updateDisplayNote(){const gain=Number($('#motion-gain').value);$('#display-note').textContent=gain===1?'Angles and displacement shown at actual scale.':`${gain}× visual magnification. All numbers and exports stay at actual scale.`;$('#view-note').textContent=`${run?modeLabel(run.config):'Walking'} · ${gain===1?'actual scale':`MAGNIFIED ${gain}×`}`;$('#display-note').classList.toggle('magnified',gain!==1);}
...
function updateStats(){
 ...
 $('#compute-note').textContent=`${live&&!run.complete?(playing?'Live physics':'Live paused'):run.complete?'Full trajectory recorded':'Recorded prefix'} · ${until.toFixed(2)} / ${c.numerics.duration_s} s calculated · ${run.samples.length.toLocaleString()} outputs${dirty?' · Unapplied edits':''}`;
}
